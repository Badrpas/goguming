...
func GetCameraPos(c *camera.Camera) cp.Vector {
	return cp.Vector{c.X, c.Y}
}

func TranslatePosFromCamera(c *camera.Camera, pos cp.Vector) cp.Vector {
	camPos := GetCameraPos(c)
	w, h := c.Width, c.Height
	s := cp.Vector{float64(w) / c.Scale / 2, float64(h) / c.Scale / 2}

	return pos.Sub(camPos).Add(s)
}
